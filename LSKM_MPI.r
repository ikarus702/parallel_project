...
d_D_sum <- crossprod(dz,dz)
  ...
  for(g.iter in 1:g.Iter){
    ...
    #    }
        ...
        g[1,] <- as.numeric(-0.5*sum(diag(d_dV_rho %*% d_P))+0.5*t(dy) %*% d_P %*% d_dV_rho %*% d_P %*% dy) 
        g[2,] <- as.numeric(-0.5(sum(diag(P)))+0.5*t(dy) %*% d_P %*% d_P %*% dy)
        

        
        H_psi[1,1] <- as.numeric(0.5*(sum(diag(d_dV_rho %*% d_P %*% d_dV_rho %*% d_P - d_dV2_rho %*% d_P)))
                                 -0.5*t(dy) %*% d_P %*% (2*d_dV_rho %*% d_P %*% d_dV_rho - d_dV2_rho) %*% d_P %*% dy)
        
        H_psi[1,2] <- as.numeric(0.5*(sum(diag(d_P %*% d_dV_rho %*% d_P)))-0.5*t(dy) %*% d_P %*% (d_P %*% d_dV_rho+ d_dV_rho %*% d_P) %*% d_P%*% dy)
        
        H_psi[2,1] <- H_psi[1,2]
        
        H_psi[2,2] <- as.numeric(0.5* sum(diag(d_P %*% d_P))-t(dy) %*% d_P %*% d_P %*% d_P %*% dy)
        
        if(!is.positive.definite(H.psi)){
          H_psi <- H_psi + 10^(-6)*diag(2)
        }
        ...
        if ('try-error' %in% class(inv.H.psi)){
          ...
          new.psi <- old.psi - solve(H_psi) %*% g_psi
        ...
        d_A <- d_inv_tmp %*% (inv.lambda*d_poly_K + dX %*% solve(t(X) %*% d_inv_tmp %*% dX) %*% t(dX) %*% d_inv_tmp)
        ...
        new.psi[2,] <- as.numeric(t(dy - d_A %*% dy) %*% (dy - d_A %*% dy)/(n-sum(diag(d_A))))
        
        #loglik[iter] <- -0.5*(log(det(V))+t(scale.y)%*%P%*%scale.y)
        if(det(V) < 0){
          
          loglik[iter] <- as.numeric(-0.5*(log(det(inv_XVX))+t(dy) %*% d_P %*% dy))
       
        }else if(det(inv_XVX)<0){ 
       
          loglik[iter] <- as.numeric(-0.5*(log(det(V))+t(dy) %*% d_P %*% dy))
          
        }else if(det(V) <0 & det(inv_XVX)<0){
          
          loglik[iter] <- as.numeric(-0.5*(t(dy) %*% d_P %*% dy))
        }else{
          
          loglik[iter] <- as.numeric(-0.5*(log(det(V))+log(det(inv_XVX)) + t(dy) %*% d_P %*% dy))
